...
def generate_ultra_clean_data(tmp_path):
    """Generates 4 perfectly clean reads with highly visible sequences."""
...
def test_ultra_clean(tmp_path):
    fasta, csv, fastq, out_bam, expected_qnames = generate_ultra_clean_data(tmp_path)
    ...
    cmd = [
        ...
        "--minimum_distance", "0"  # Zero edits allowed!
    ]
    ...
    if missing_qnames or correct_templates != 4 or correct_strands != 4:
        ...
        pytest.fail(
            f"\n\n--- ULTRA-CLEAN TEST FAILED ---\n"
            f"Expected 4 reads to map perfectly.\n"
    ...
    # --- SUCCESS REPORT ---
    # We force print this to the terminal so a human can actually read it
    print("\n\n" + "=" * 50)
    print("🟢 ULTRA-CLEAN TEST PASSED 🟢")
    print("=" * 50)
    print(f"✅ Total reads expected: {len(expected_qnames)}")
    print(f"✅ Total reads mapped: {len(mapped_qnames)} / {len(expected_qnames)}")
